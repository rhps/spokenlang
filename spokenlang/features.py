...
from spokenlang.constants import *
...
def process_audio(input_dir, debug=False):
    ...
    for extension in extensions:
        # Reading all file with the extensions in input_dir
        files.extend(glob.glob(os.path.join(input_dir, extension)))

    for file in files:
        if debug:
            # Debug file with one example files
            file = ('../datasets/test/', 'de_f_63f5b79c76cf5a1a4bbd1c40f54b166e.fragment1.flac')
            start = time.time()
        ...
        np.savez_compressed(file_without_ext + '.mel', data=mel)
